...
def __update_price_Log_eth(ea):
    supply = totalSupply(ea, const.contract_CFToken_eth[ea.host])
    price = getVirtualPrice(ea, const.contract_CFVault_eth[ea.host])
    current_time = ea.get_block_time()  #
    price_log = PriceLogsEth(cff_amount=supply,
                              virtual_price=price,
...
def __update_token_profile_eth(address, amount):

    query = db.session.query(TokenProfile_eth).filter(
        TokenProfile_eth.address == address).all()
    if not query:
        new_token_address = TokenProfile_eth(address=address, amount=amount)
        db.session.merge(new_token_address)
        db.session.commit()
    else:
        assert len(query) == 1
        new_token_address = TokenProfile_eth(id=query[0].id,
                                              address=address,
...
def calibrate_token_balance_eth(ea):
    query = db.session.query(TokenProfile_eth).all()
    for item in query:
        __update_token_profile_eth(
            item.address,
            int(
                balanceOf(ea, const.contract_CFToken_eth[ea.host],
                          '0x%s' % item.address)) - int(item.amount))


def initial_token_address_table_eth():

    query = db.session.query(UserLogsEth).filter(
        UserLogsEth.operation_type == 0).all()
    for item in query:
        __update_token_profile_eth(item.address, int(item.cff_amount))
    query = db.session.query(UserLogsEth).filter(
        UserLogsEth.operation_type == 1).all()
    for item in query:
        __update_token_profile_eth(item.address, -int(item.cff_amount))

    query = db.session.query(TransferLogsEth).all()
    for item in query:
        __update_token_profile_eth(item.from_address, -int(item.cff_amount))
        __update_token_profile_eth(item.to_address, int(item.cff_amount))
...
def __handle_CUserdeposit_eth(data, txh, txtime, address_from):
    query = db.session.query(UserLogsEth).filter(
        UserLogsEth.hash == txh[2:]).all()
    if not query:
        ...
        userlogs = UserLogsEth(address=address,
                                hash=txh[2:],
        ...
        db.session.commit()
        __update_token_profile_eth(address, int(cff_amount))


def __handle_CUserwithdraw_eth(data, txh, txtime):
    ...
    if not query:
        ...
        userlogs = UserLogsEth(address=address.lower(),
                                hash=txh[2:],
                                usdc_amount=usdc_amount,
                                cff_amount=cff_amount,
                                usdc_fee=usdc_fee,
        ...
        db.session.merge(userlogs)
        db.session.commit()
        __update_token_profile_eth(address.lower(), -int(cff_amount))


def __handle_BidInfo_eth(change, txtime, ea):
    ...
    op_code = {
        ...
        '__handle_CUserwithdraw_eth(data,transactionhash.hex(),txtime)',
        const.topic_CUserDeposit:
        '__handle_CUserdeposit_eth(data,transactionhash.hex(),txtime,deposit_address)',

        const.topic_CUserTransfer:
        '__handle_CUserTransfer_eth(data,transactionhash.hex(),txtime,from_address,to_address)'
    }
    eval(op_code[event])


def handle_BidInfo_eth(ea, bid_filter):

    changes = ea.get_filter_changes(bid_filter.filter_id)
    if changes:
        [
            __handle_BidInfo_eth(change, ea.get_block_time(), ea)
            for change in changes
        ]


def __handle_CUserTransfer_eth(data, transactionhash, txtime, from_address,
                                to_address):

    if (from_address != const.address_of_zero[2:]) and (
            to_address != const.address_of_zero[2:]) and (from_address != const.contract_ERC20DepositApprover_eth[2:].lower()) and (to_address != const.contract_ERC20DepositApprover_eth[2:].lower()) and (from_address != const.contract_upgrade_eth[2:].lower()) and (to_address != const.contract_upgrade_eth[2:].lower() ):

        query = db.session.query(TransferLogsEth).filter(
            TransferLogsEth.hash == transactionhash[2:]).all()
        ...
        if not query:
            ...
            transferlogs = TransferLogsEth(from_address=from_address.lower(),
            ...
            db.session.commit()
            __update_token_profile_eth(from_address.lower(), -int(cff_amount))
            __update_token_profile_eth(to_address, int(cff_amount))


def handle_Supply_info_eth(ea):
    supply = totalSupply(ea, const.contract_CFToken_eth[ea.host])
    price = getVirtualPrice(ea, const.contract_CFVault_eth[ea.host])
    ...
    if (timestamp_to_utcdate(db_date).day !=
        ...
        query = db.session.query(PriceHistoryEth).filter(
            PriceHistoryEth.pub_date == start_time_date).all()
        if not query:

            query = db.session.query(UserLogsEth).filter(
                UserLogsEth.pub_date < db_date).filter(
                    UserLogsEth.pub_date >= start_time_date).all()
            ret = 0
            for item in query:
            ...
            query = db.session.query(PriceHistoryEth).order_by(
            ...
            price_history = PriceHistoryEth(cff_amount=totalS,
            ...
            query = db.session.query(ProfitLogsEth).filter(
                ProfitLogsEth.pub_date == start_time_date).all()

            if not query:
                profit_ratio  =0 if profit_ratio < 0 else profit_ratio
                profitlog = ProfitLogsEth(profit_usdc=ret,
